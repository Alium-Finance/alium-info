...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://exchange.alium.finance/#/swap?inputCurrency=${token0Address}`
  } else {
    return `https://exchange.alium.finance/#/swap?inputCurrency=${
      token0Address === '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' ? 'ETH' : token0Address
...
export function formattedPercent(percent, useBrackets = false) {
  ...
  if (percent < 0.0001 && percent > 0) {
    return (
      <Text fontWeight={500} color="#24BA7B">
        {'< 0.0001%'}
      </Text>
    )
  }
  ...
  if (percent < 0 && percent > -0.0001) {
    return (
      <Text fontWeight={500} color="#F04628">
        {'< 0.0001%'}
      </Text>
    )
  }
  ...
  if (fixedPercent > 0) {
    if (fixedPercent > 100) {
      return <Text fontWeight={500} color="#24BA7B">{`+${percent?.toFixed(0).toLocaleString()}%`}</Text>
    } else {
      return <Text fontWeight={500} color="#24BA7B">{`+${fixedPercent}%`}</Text>
    }
  } else {
    return <Text fontWeight={500} color="#F04628">{`${fixedPercent}%`}</Text>
  }
}
