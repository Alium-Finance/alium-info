...
const Container = styled.div`
  ...
  ${({big})=> big ? 'height: 58px; flex-basis: 100%;' : ''}
...
const Menu = styled.div`
  ...
  left: 0;
  padding-bottom: 20px;
  background: #fff; // ${({ theme }) => theme.bg6};
...
const Heading = styled(Row)`
  ...
  ${({disableBorder}) => disableBorder ? '' : 'border-bottom: 1px solid #EBEDF9;'}
`
...
export const Search = ({ small = false, disableMargin, big }) => {
  ...
  return (
    <Container small={small} disableMargin={disableMargin} big={big}>
      ...
      <Menu hide={!showMenu} ref={menuRef}>
        ...
        <div>
          {filteredPairList && Object.keys(filteredPairList).length === 0 && (
            <MenuItem>
              <TYPE.body style={{color: '#8990A5'}}>No results</TYPE.body>
            </MenuItem>
          )}
          {filteredPairList &&
