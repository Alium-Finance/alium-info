...
function PairList({ pairs, color, disbaleLinks, maxItems = 10 }) {
  ...
  const ListItem = ({ pairAddress, index }) => {
    ...
    if (pairData && pairData.token0 && pairData.token1) {
      ...
      return (
        <DashGrid style={{ height: '55px' }} disbaleLinks={disbaleLinks} focus={true}>
          <DataText area="name" fontWeight="500">
            ...
            <DoubleTokenLogo
              ...
              margin={!below740}
            />
            <CustomLink style={{ marginLeft: '9px', whiteSpace: 'nowrap' }} to={'/pair/' + pairAddress} color={color}>
              <FormattedName
                text={pairData.token0.symbol + '-' + pairData.token1.symbol}
                maxCharacters={below600 ? 8 : 16}
                adjustSize={true}
                link={true}
                style={{ color: '#6C5DD3', fontWeight: '500' }}
              />
            </CustomLink>
          </DataText>
          ...
          {!below1080 && <DataText area="apy" style={{justifyContent: 'flex-end'}}>{apy}</DataText>}
        </DashGrid>
      )
    } else {
      return ''
