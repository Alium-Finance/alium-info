...
const List = styled(Box)`
  ...
  padding: 2px 22px 31px 23px !important;
`
...
const SortText = styled.button`
  ...
  letter-spacing: 1px;
  padding: 0px;
  color: ${({ active, theme }) => (active ? theme.text1 : '#8990A5')};
  outline: none;
  font-weight: 700;
    ...
    return (
      <DashGrid style={{ height: '56px' }}>
        <DataText area="txn" fontWeight="500">
          <Link color={'#6C5DD3'} external href={urls.showTransaction(item.hash)}>
        ...
        {!below1080 && (
          <DataText area="account">
            <Link color={'#6C5DD3'} external href={'https://bscscan.com/address/' + item.account}>
              {item.account && item.account.slice(0, 6) + '...' + item.account.slice(38, 42)}
            </Link>
          </DataText>
        )}
        <DataText area="time" style={{justifyContent: 'flex-end'}}>{formatTime(item.timestamp)}</DataText>
      </DashGrid>
    )
  }
        ...
        <>
          ...
          {!below1080 && (
            <Flex alignItems="center" justifyContent="left">
              <TYPE.body
                area="account"
                style={{ color: '#0B1359', textTransform: 'uppercase', fontSize: '12px', fontWeight: '700' }}
              >
                Account
              </TYPE.body>
            </Flex>
